...
import { useCreateSeatBookingMutation, useDeleteSeatBookingMutation } from '../../../services/api'
import toast from 'react-hot-toast';
import { timeslots } from '../../home/book-a-seat'


export const SeatBooking = (props) => {
  const { data: { locations, seatBookings } = {} } = props
  ...
  useEffect(() => {
    if (selectedCenter.id && selectedDate && selectedTimeSlot.id) {
      setSeats([...Array(selectedCenter.seats).keys()].map(index => ({
        id: index + 1,
        status: selectedCenter.seatBooking
          .find(booking => {
            return booking.seatNumber === index + 1 &&
              new Date(booking.Date).toLocaleDateString() == selectedDate.toLocaleDateString() &&
  ...
  //     const newBookedSeats = bookedSeats.filter(booking => booking.id !== delete_result.data.deleted.id)
  ...
  const handleBookSeat = async () => {
    if (!selectedSeat || !selectedDate || !selectedTimeSlot.id || !selectedCenter.id || !email) {
      setErrorMessage("Please select an applicant, center, seat, date, and time slot to book.");
      return;
    }
    ...
    const checkSeatAvailable = seatBookings.find(booking => {
      return (
        ...
        new Date(booking.Date).toLocaleDateString() == bookedSeat.Date.toLocaleDateString() &&
    ...
    if (checkSeatAvailable) {
      checkSeatAvailable && setErrorMessage("This seat is already booked for this center, date and time slot.");
      // checkTimeslot && setErrorMessage("You have already booked a seat for this timeslot")
      return;
    }
    ...
    if (result.data?.message === "success") {
      setSeats([])
      // setBookedSeats([...bookedSeats, { ...bookedSeat, id: result.data.seatBooking.id }]);
      setSelectedSeat({ id: '' });
    ...
    <>
      ...
      <Box>
        <Grid
          ...
          sx={{ mt: 3 }}
        >
          <TextField fullWidth label='Applicant Email' value={email} onChange={(e) => setEmail(e.target.value)}/>
        </Grid>
        <Grid
          item
          md={12}
          ...
          sx={{ mt: 3 }}
        >
          <TextField fullWidth label='Select Center' value={selectedCenter.id} select onChange={(e) => handleCenterChange(e)}>
        ...
        >
          <TextField
            ...
            inputProps={{
              ...
              max: new Date((+new Date) + 2678400000).toISOString().split('T')[0]
            }}
          ></TextField>
        </Grid>
        <Grid
          ...
          sx={{ my: 3 }}
        >
          <TextField fullWidth label='Select Time' value={selectedTimeSlot.id} select onChange={handleTimeSlotChange}>
        ...
        <Box sx={{ my: 5 }}>
          <Button variant="contained" fullWidth onClick={handleBookSeat}>Book Seat</Button>
        ...
        <ul>
          {bookedSeats.map((bookedSeat, index) => (
            <li key={bookedSeat.id}>
              Center: {bookedSeat.center.name} - Seat: {bookedSeat.seatNumber} - Date: {new Date(bookedSeat.Date).toLocaleDateString()} -
              Time: {timeslots.find(t => t.id === bookedSeat.timeslot).time}
              <Button variant="outlined" onClick={()=> handleDeleteBooking(bookedSeat.id, index)}>Cancel</Button>
            </li>
          ))}
        </ul> */}
      </Box>
