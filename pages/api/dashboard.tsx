...
/**
 * Fetch dashboard data (extracted for caching)
 */
async function fetchDashboardData(cohortId: string | undefined): Promise<any> {
...
export default async function handler(
  ...
  try {
    ...
    const cacheKey = CacheKeys.dashboard(cohortId);
    const cacheTtl = parseInt(process.env.CACHE_TTL_DASHBOARD || '300', 10);

    // Fetch dashboard data with caching
    const responseData = await cacheManager.getOrSetWithLock(
      cacheKey,
      () => fetchDashboardData(cohortId),
      cacheTtl,
      60 // Lock TTL of 60 seconds to prevent cache stampede
    );
...
export const config = {
  maxDuration: 300, // 5 minutes - allows for cache misses on large datasets
};
