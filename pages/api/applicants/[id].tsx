...
export default async function handler(
  ...
  if (req.method === 'PATCH') {
    ...
    const refObject = referrer ? Object.keys(referrer).length ? {
      referrer: {
        upsert: {
          update: {
            ...referrer
    ...
    try {
      ...
      return res.status(202).send({ message: "Applicant Updated", applicant: user })
    } catch (err) {
      console.error(err)
  ...
  try {
    ...
    if (token?.userData?.role === 'APPLICANT' && token?.userData?.userId !== _user.id) {
      return res.status(403).json({ message: "Unauthorized" });
    }
