...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Get cohorts with pagination and COHORT COURSES INCLUDED
    ...
    // Serialize BigInt values in cohortCourses
    const serializedCohorts = cohorts.map(cohort => ({
      ...
      cohortCourses: cohort.cohortCourses?.map(cc => ({
        ...
        course: cc.course ? {
          ...cc.course,
          id: cc.course.id.toString(), // Convert BigInt course.id to string
        } : null
      })) || []
    }));
